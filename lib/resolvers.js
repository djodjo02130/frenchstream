...
async function resolveFilmoon(embedUrl) {
    // Parse video code from embed URL: https://bysebuho.com/e/XXXXX
    const codeMatch = embedUrl.match(/\/e\/([a-zA-Z0-9]+)/);
    ...
    const detailsResp = await fetch(`${embedOrigin}/api/videos/${videoCode}/embed/details`, {
        headers: { ...apiHeaders, Referer: embedUrl, 'x-embed-parent': embedUrl },
    });
    ...
    await fetch(`${playerOrigin}/api/videos/${videoCode}/embed/settings`, {
        headers: { ...apiHeaders, Referer: playerUrl, 'x-embed-parent': embedUrl },
    });
    ...
    if (!challenge.challenge_id || !challenge.nonce) return null;
    ...
    if (!attest.token) return null;

    // Step 5: POST playback → get encrypted payload
    const playbackResp = await fetch(`${playerOrigin}/api/videos/${videoCode}/embed/playback`, {
        method: 'POST',
        headers: { ...apiHeaders, 'Content-Type': 'application/json', Referer: playerUrl, 'x-embed-parent': embedUrl },
        body: JSON.stringify({
            fingerprint: {
    ...
    if (!playbackData.playback) return null;
