...
const FILM_LANG_MAP = { default: 'VF', vostfr: 'VOSTFR', vfq: 'VFQ', vff: 'VFF' };
...
async function scrapeFilmPage(pageUrl) {
    ...
    for (const player of playerKeys) {
        ...
        // Collecter les URLs valides par langue pour ce player
        const playerStreams = {};
        for (const [langKey, url] of Object.entries(langs)) {
            if (!url || typeof url !== 'string' || !url.trim() || !url.startsWith('http')) continue;
            const lang = FILM_LANG_MAP[langKey];
            if (!lang) continue;
            playerStreams[lang] = url.trim();
        }

        // Dédupliquer : si VF a la même URL qu'une autre langue, c'est pas une vraie VF
        const vfUrl = playerStreams['VF'];
        if (vfUrl) {
            const otherLangs = Object.entries(playerStreams).filter(([l]) => l !== 'VF');
            if (otherLangs.some(([, u]) => u === vfUrl)) {
                delete playerStreams['VF'];
            }
        }

        for (const [lang, url] of Object.entries(playerStreams)) {
            streams.push({
                player,
...
async function scrapeMetadata(pageUrl) {
    ...
    // Title from <h1> or <title> — normalize whitespace, extract year
    let rawTitle = $('h1').first().text().replace(/\s+/g, ' ').trim()
