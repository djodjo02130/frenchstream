...
function extractNewsId(pageUrl) {
    const match = pageUrl.match(/\/(\d+)-/) || pageUrl.match(/[?&]newsid=(\d+)/);
 ...
 * GET {baseUrl}/engine/ajax/sx.php?p={newsId}
 * Returns array of { player, playerName, lang, url }
 */
async function scrapeSeriesPage(pageUrl, episode) {
    ...
    const apiUrl = `${utils.getBaseUrl()}/engine/ajax/sx.php?p=${newsId}`;
    const response = await fetch(apiUrl, { headers: { ...HEADERS, 'X-Requested-With': 'XMLHttpRequest' } });
    const data = await response.json();
...
/**
 * Scrape catalog page.
 * @param {string} pathSegment FS URL path: 'films', 's-tv', 'film-commu', 'serie-commu', or '' for box office (homepage)
 * @param {number} page 1-based page number
 * @param {'movie'|'series'} type Stremio type for the items
 * Returns { items: [{ id, title, poster, url, quality, version, type }], hasNext }
 */
async function scrapeCatalog(pathSegment, page, type = 'movie') {
    const cacheKey = `${pathSegment}:${page}:${type}`;
    const cached = cache.get('catalog', cacheKey);
    if (cached) return cached;

    const pageStr = page > 1 ? `page/${page}/` : '';
    const prefix = pathSegment ? `${pathSegment}/` : '';
    const url = `${utils.BASE_URL}/${prefix}${pageStr}`;
